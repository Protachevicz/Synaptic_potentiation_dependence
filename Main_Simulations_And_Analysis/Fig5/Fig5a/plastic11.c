...
int conex1[N],conex2[N],list1[N][N],list2[N][N];
...
int main(){		
				...
				c1[EQ+1],c2[EQ+1],c3[EQ+1],ydot[EQ+1],s[N+1],tdisp[N+1],wij,z[N+1], wij_max,vec[N+1][Dmax],isi[2],T;
				char file1[100];
				FILE *ff,*fg,*fw;
				wij_max=1.0;
				wij=0.001;
...
//for(jj=0;jj<=3;jj++)
{
				...
				ii=jj;
				sprintf(file1,"F_Rg_Rl_M_%d.dat",ii);
				ff=fopen(file1,"w");
				...
				for(I0=100*(ii-1);I0<100*ii;I0=I0+100.0)
				for(desvio=0;desvio<=2.02;desvio=desvio+0.01)
				{

					for(i=0;i<N;i++)
								for(j=0;j<N;j++) matrix[i][j]=wij;		
	//%%%%%%%%%%%%%%%%%%--initial-conditions--%%%%%%%%%%%
				for(i=0;i<N;i++){
								x[i]=-80;//.0+4*(ii-3)*(ii-2)*(ii-1)*(desvio*1.0)*ran1(&idum);//+1*(ii-3)*(ii-2)*(ii-1)*ii*ran1(&idum); 
								x[i+N]=b+desvio*gasdev(& idum);//+4*(ii-2)*(ii-1)*ii*ran1(&idum)+20*(ii-3)*(ii-2)*(ii-1)*ii*ran1(&idum);		
//%%%%%%%%%%%%%%%%%%%%%%%
                                                                if(x[i+N]<0) x[i+N]=0;
								y[i]=x[i];
								y[i+N]=x[i+N];
...
///////// Parametro de Ordem Global
			...
			if(m>2)		if(isi[0]>0.0)	{
				...
				x[18]= (x[0]-wij)/(x[1]*T);
				if(x[1]<0.000000000001) x[18]=0;

				//fprintf(ff,"%d\t%f\t%f\t%f\n",ii+1,x[0],x[4],x[3]);
				                                        //      	 M,   SD_M, RG,  F,   RL
				fprintf(ff,"%f\t%f\t%f\t%f\t%f\t%f\t%f\t%.12f\n",I0,desvio, x[0],x[4],x[6],x[1],x[17],x[18]);
				printf("Corrent = %f, Desvio = %f, Media= %f,  desvio_m =%f, Rg = %f, F = %f, Rl=%f, P =(Media_f-Media_i)/(F*T)=%f\n \n",I0,desvio, x[0],x[4],x[6],x[1], x[17], x[18]);
				//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
				//// M - x[0]
				//// SD_M - x[4]
				//// RG - X[6]
